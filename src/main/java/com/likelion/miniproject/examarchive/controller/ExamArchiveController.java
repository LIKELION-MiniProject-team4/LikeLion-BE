...
public class ExamArchiveController {
    ...
    @PostMapping("/api/professors/{professorId}/exam-archives")
    public ResponseEntity<GlobalApiResponse<ExamArchiveContentResponse>> write(
            @AuthenticationPrincipal AuthUser authUser,
            @PathVariable Long professorId,
            @Valid @RequestBody ExamArchiveWriteRequest request
    ) {
        ExamArchiveContentResponse result = examArchiveService.write(authUser.userId(), professorId, request);
        return ResponseEntity.status(201).body(GlobalApiResponse.created(ExamArchiveResponseCode.EXAM_ARCHIVE_CREATED, result));
    }

    /** 본인 또는 관리자만 삭제 가능 */
    @DeleteMapping("/api/exam-archives/{examArchiveId}")
    public ResponseEntity<GlobalApiResponse<Void>> delete(
            @AuthenticationPrincipal AuthUser authUser,
            @PathVariable Long examArchiveId
    ) {
        examArchiveService.delete(authUser.userId(), authUser.role(), examArchiveId);
        return ResponseEntity.ok(GlobalApiResponse.ok(ExamArchiveResponseCode.EXAM_ARCHIVE_DELETED));
    }
}